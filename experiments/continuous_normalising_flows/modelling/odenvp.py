...
from .layers.elemwise import LogitTransform, ZeroMeanTransform
from .layers.container import SequentialFlow
from .layers.odefunc import ODEnet, ODEfunc
from .layers.cnf import CNF
from .layers.squeeze import SqueezeLayer
import numpy as np
...
class ODENVP(nn.Module):
    ...
    def __init__(
        ...
        print("Using %d scales" % self.n_scale)
        ...
        for i in range(self.n_scale):
            transforms.append(
                StackedCNFLayers(
                    ...
                    init_layer=(LogitTransform(self.alpha) if self.alpha > 0 else ZeroMeanTransform())
